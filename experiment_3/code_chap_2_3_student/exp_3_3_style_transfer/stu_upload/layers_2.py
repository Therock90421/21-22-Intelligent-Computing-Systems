...
class ConvolutionalLayer(object):
            ...
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        self.output[idxn, idxc, idxh, idxw] = np.sum(np.multiply(self.weight[:,:,:,idxc], self.input_pad[idxn, :, idxh * self.stride:idxh * self.stride + self.kernel_size, idxw * self.stride:idxw * self.stride + self.kernel_size])) + self.bias[idxc]
    ...
    def forward_speedup(self, input):
        ...
        self.output = (self.output_col.reshape(self.input.shape[0], height_out, width_out, self.channel_out) + self.bias).transpose(0, 3, 1, 2)# [N, C, H, W]
        self.forward_time = time.time() - start_time
        return self.output
    def backward_speedup(self, top_diff):# [N, C, H, W]
        # TODO: 改进backward函数，使得计算加速   实际上不需要计算weight和bias的梯度，只需要计算到X的梯度即可
        start_time = time.time()
        ...
        top_diff_pad = np.zeros([top_diff.shape[0], top_diff.shape[1], top_diff.shape[2]+2*self.kernel_size-2, top_diff.shape[3]+2*self.kernel_size-2])# [N, C, H+2K-2, W+2K-2]
        top_diff_pad[:, :, self.kernel_size-1:1-self.kernel_size, self.kernel_size-1:1-self.kernel_size] = top_diff
        top_diff_pad_col = np.zeros([self.input.shape[0], self.input_pad.shape[2]*self.input_pad.shape[3], top_diff.shape[1]*(self.kernel_size**2)]) # [N, HW, K^2C]
